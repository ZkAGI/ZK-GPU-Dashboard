...
export function SidebarItem(props: SidebarItemProps) {
  ...
  const content = (
    <Link href={href || "#"} passHref>
      ...
      >
        {icon && (
          <div className={`[&>svg]:size-4 ${disabled ? "text-gray-500" : ""}`}>
            {icon}
          </div>
        )}
        <p>{name}</p>
  ...
  return disabled ? (
    <Tooltips message="Coming Soon">{content}</Tooltips>
  ) : (
    content
  );
}
            ...
            <div className="flex flex-col rounded-lg m-2 px-3 py-5 relative">
              ...
              <div className="flex flex-row justify-between mb-5 z-50 text-white mt-2 bg-">
                ...
                <div className="size-7 bg-white rounded-md flex justify-center items-center mt-1">
                  <div className="size-4 rounded-full bg-blue-600 text-xs text-center">
                    ?
                  </div>
                </div>
              </div>
              <div className="flex justify-center items-center z-50">
                <a
                  href="https://zkagi.gitbook.io/introduction"
                  className="bg-[#000D33] rounded-lg p-2 w-full text-sm text-center text-white"
                >
                  DOCUMENTATION
                </a>
              </div>
